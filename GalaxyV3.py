...
starforming_chance = 1.6 #The global starforming probability- it is modified in SPSF function
...
def Ring_Velocity_Maker(x):
    ...
    return  (2*math.pi/(2*x+1))  #the last coefficient affects the "speed"
...
def Update_Galaxy(index):
        ...
        for j in range(len(Galaxy[i])):
            ...
            if Galaxy[i][j][age] == 0:
                ...
                if Galaxy[i][j][age] > 24:
                    ...
                    Galaxy[i][j][age] += 1    
                elif Galaxy[i][j][age] > 13 and Galaxy[i][j][age] < 26:
    ...
    starburst_ratio = count/(i*j)
    print('Active Cells %d    Starburst Ratio  %.2f' % (count,starburst_ratio))
    if starburst_ratio >.18:
        starforming_chance -= .05   
        starforming_flag = True
        print('StarFormation Rate Adjusted down to %.2f ' % starforming_chance)
    if starburst_ratio <.16 and starforming_flag:
        starforming_chance += .05
...
def SPFS(current_ring, current_cell):
    ...
    if current_ring > 0 and current_ring < len(Galaxy): #extra protection... not needed I think..but..
        ...
        rng.seed()
        if rng.random() < .06*starforming_chance:
...
def Finalize_Galaxy():
    ...
    for i in range(len(Galaxy)):
        for j in range(len(Galaxy[i])):
            if i < 10:
                print(i,j,Galaxy[i][j][age],Galaxy[i][j][theta])
